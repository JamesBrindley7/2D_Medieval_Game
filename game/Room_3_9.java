...
class Room_3_9 extends Room {
    ...
    private Texture creditsTex = new Texture();
     ...
     */
    Room_3_9(RenderWindow window, KeyboardHandler kh, Level level) throws IOException {
        ...
        Path path = Paths.get("sprites/Room_v2_Door_Left_Locked.png");
        Path credits = Paths.get("sprites/Credits.png");

        Texture background = new Texture();
        background.loadFromFile(path);
        creditsTex.loadFromFile(credits);
        ...
        bossName = new Text("King Andrew Scott III", font, 48);
        ...
        player = new Player(window, 250, 300, testGuard, this);
        boss = new Boss3(window, 600, 500, this);
    ...
    void currentRoomLoop() throws IOException {
        while (window.isOpen() && (currentRoom)) {
            ...
            if (credits) {
                backgroundRoom.setTexture(creditsTex);
                fadeIn.draw();
                window.draw(backgroundRoom);
                if (kh.escapePressed()) {
                    System.exit(0);
